...
def main():
    ...
    # Lưu file paths trong session state để sử dụng khi nhấn nút
    if 'file_paths' not in st.session_state:
        st.session_state.file_paths = []

    # Nút xử lý file
    process_button = st.sidebar.button("Xử lý File")
    ...
    if uploaded_files:
        ...
        # Cập nhật file paths vào session state
        st.session_state.file_paths = file_paths
        st.sidebar.write(f"Đã chọn {len(file_paths)} file để xử lý.")

    if not st.session_state.file_paths and not process_button:
        st.error("Vui lòng tải lên ít nhất một file CSV hoặc file ZIP chứa các file CSV và nhấn 'Xử lý File'.")
        return

    try:
        # Chỉ xử lý khi nhấn nút "Xử lý File"
        if process_button and st.session_state.file_paths:
            unique_df, raw_df = load_data(st.session_state.file_paths)
