...
# Hàm lấy danh sách thư mục con trong thư mục input
def get_subdirectories(directory):
    subdirs = [f.name for f in os.scandir(directory) if f.is_dir()]
    return sorted(subdirs)

# Hàm lấy danh sách file CSV từ thư mục
...
def main():
    ...
    input_dir = r"D:\Auto_PPC\input"
    if not os.path.exists(input_dir):
        os.makedirs(input_dir)

    subdirectories = get_subdirectories(input_dir)
    if not subdirectories:
        st.error("Không tìm thấy thư mục con nào trong thư mục 'input'. Vui lòng tạo các thư mục như 'mom', 'dad', v.v.")
        return

    st.header("Chọn Dữ Liệu Theo Thư Mục")
    selected_subdir = st.selectbox("Chọn thư mục", subdirectories)
    selected_dir = os.path.join(input_dir, selected_subdir)
    input_file_paths = get_csv_files_from_directory(selected_dir)
    if input_file_paths:
        st.sidebar.write(f"Các file CSV được tìm thấy trong thư mục '{selected_subdir}':")
        for file_path in input_file_paths:
            st.sidebar.write(f"- {os.path.basename(file_path)}")
    else:
        st.sidebar.warning(f"Không tìm thấy file CSV nào trong thư mục '{selected_subdir}'.")

    uploaded_files = st.sidebar.file_uploader("Hoặc upload file CSV", type=["csv"], accept_multiple_files=True)
    ...
    file_paths = input_file_paths + uploaded_file_paths
    if not file_paths:
        st.error(f"Vui lòng thêm file CSV vào thư mục '{selected_subdir}' hoặc upload file để tiếp tục.")
        return
    ...
    if st.sidebar.button("Tạo PDF"):
        ...
        pdf_path = os.path.join(output_dir, f"report_{selected_subdir}_{datetime.now().strftime('%Y%m%d_%H%M%S')}.pdf")
